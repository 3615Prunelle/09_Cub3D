...
"Cub3D is a 42 project where you must create a “realistic” 3D graphical representation of the inside of a maze from a first-person perspective.  
You have to create this representation using the ray-casting principles."  

# Instructions
If you're here, you already know the drill : git clone, make, enjoy ! And ... That's pretty much it !  
When executing the game (called cub3d), you have to fetch the scene description (in `.cub` format) of your choice in the `./scene_descriptions/valid`.  
If you feel creative, you can change the emojis (pick & choose from the texture folder and add the path in the scene description), or get new ones (gmail -> Compose new mail -> choose & select emoji -> save in the texture folder), edit the scene description files, or even create your own, as long as it respects the same synthax, and that all required elements are provided (you'll figure it out, it's pretty straightforward).  
...
> Also ! If you wanna play around with the size of the map, the minimap or some technical rendering stuff, feel free to edit the `cub3d.h` file. Here are the variables you can edit :
> * `# define VIEW_WIDTH / VIEW_HEIGHT` >> Size of the game window itself
> * `# define MINI_WIDTH / MINI_HEIGHT ` >> Size of the minimap within the game window
> * `# define MAP_SCALE` >> Minimap zoom level: smaller value = wider map view
> * `# define VIEW_DISTANCE` >> Visual maximum distance between the player and the wall
